...
// Send a plain text message directly via WhatsApp Cloud API
...
// Hand off to human agent — add "human_agent" label
async function handoffToHuman(conversationId) {
  // Get current labels
  const convResponse = await api.get(`/conversations/${conversationId}`);
  const currentLabels = convResponse.data?.labels || [];

  // Add "human_agent" label
  if (!currentLabels.includes("human_agent")) {
    await api.post(`/conversations/${conversationId}/labels`, {
      labels: [...currentLabels, "human_agent"],
    });
  }
}
